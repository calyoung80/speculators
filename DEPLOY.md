...
> 容器内 `/vllm-workspace/vllm` 和 `/vllm-workspace/vllm-ascend` 已有 git 仓库， 只需 `git remote add fork git@github.com:calyoung80/xxx.git` 然后 `git fetch fork && git checkout fork/dspark-ascend-patches`。
